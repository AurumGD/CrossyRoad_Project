...
class Game {
  ...
  public:
	...
	void scoreLayout(int lives, int score){
		SetConsoleCursorPosition(hConsole, {0, 22}); 
        setColor(11);
        cout << "LIVES: ";
	...
	void scoreSystem(int y, int &highestY) {
		if (y < highestY) {
			score += 10;
			highestY = y;
		}
	...
	void loseLife() {
		...
		if (lives > 0) {
			SetConsoleCursorPosition(hConsole, {0, 22});
			setColor(12);
			cout << "You lost a life! Remaining: " << lives << "   ";
			Sleep(2000);
		}
		else {
			myMusic.stop();
			...
			while (true){
				...
				if (hasNumber || hasSymbol) cout << "Invalid name! Please input another name!"; 
				else {
					players.saveScore(playerName, score);
					players.display();
				}
			}
			players.display();
	...
	void GameRunning() {
		...
		while (playing) {
			...
			scoreLayout(lives, score);
			
			name.playerMovement(playerX, playerY);
			scoreSystem(playerY, highestY);
